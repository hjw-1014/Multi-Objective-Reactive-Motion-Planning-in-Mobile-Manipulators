...
class Tiago_OSC:
    ...
    def __init__(self, start_point=None):
        ...
        difference_x = self.end_point[0] - self.start_point[0]  # Position X difference btw start and end point
        difference_y = self.end_point[1] - self.start_point[1]  # Position Y difference btw start and end point
        difference_z = self.end_point[2] - self.start_point[2]  # Position Z difference btw start and end point
        ...
        for k in range(self.traj_points_num):
            ...
            self.start_point[0] = self.start_point[0] + (difference_x / (self.traj_points_num - 1))
            self.start_point[1] = self.start_point[1] + (difference_y / (self.traj_points_num - 1))
            self.start_point[2] = self.start_point[2] + (difference_z / (self.traj_points_num - 1))
        self.start_point = np.array([0.1, -0.7, 0.7])

        self.x_des = pin.SE3(self.qua, self.start_point)  # TODO: set desired position and orientation
    ...
    def start_simulation(self):
        ...
        for ii in range(self.number_iteration):
            ...
            print("============= Start Update ", self.x_des, '=============', ii, "th =============")
            ...
            v_ew = self.calculate_vtl(state=state, R=R)  # TODO: Velocity control
            ...
            X_w, Y_w, Z_w = self.calculate_xyz_world(self.se3ToTransfrom(self.robot.data.oMf[self.EE_idx]))
            ...
            dMi = self.x_des.actInv(self.robot.data.oMf[self.EE_idx])  # Transformation from current EE to desired EE
            #err = np.linalg.norm(pin.log6(dMi).vector)
            err = abs(np.sum(self.robot.data.oMf[self.EE_idx].translation - self.x_des.translation))
            distance = np.sqrt(np.sum(np.power(np.abs(self.robot.data.oMf[self.EE_idx].translation - self.x_des.translation), 2)))
