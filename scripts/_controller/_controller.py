...
def cascade_control_all_points(self, min_x=-1.5, min_y=-1.5, max_x=1.5, max_y=1.5,
    ...
    for i in range(len(grid_map)):  # Transverse all the points in the map
        ...
        cur_dist_paths = self.compute_current_distance_from_path_points_viz(
            cur_pos_in_list)  # For cuurent postion, get distance paths
        min_dist, min_dist_index = self.choose_min_dist_point_viz_vector(
        ...
        attractive_force = self.compute_attractive_potential_force(current_position=cur_pos,
